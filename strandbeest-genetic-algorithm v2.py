...
class SimuladorMecanismo:
    ...
    def __init__(self, comprimentos_links=None):
        ...
        self.ponto_fixo2 = np.array(
            [self.comprimentos_links["h"], 0]
        )  # Primeiro pivô fixo
        self.ponto_fixo3 = np.array(
            [self.comprimentos_links["i"], 0]
        )  # Segundo pivô fixo
...
class OtimizadorGenetico:
    ...
    def selecionar_pais(self, pontuacoes_aptidao):
        ...
        pontuacao_max = (
            max(pontuacoes_aptidao) + 1
        )  # Adiciona 1 para evitar divisão por zero
        probs_selecao = [
            (pontuacao_max - pontuacao) / pontuacao_max
            for pontuacao in pontuacoes_aptidao
        ]
        ...
        indices_pais = np.random.choice(
            range(len(self.populacao)), size=2, p=probs_selecao, replace=False
        )
    ...
    def executar_evolucao(self, geracoes=50):
        ...
        for ger in range(geracoes):
            ...
            print(
                f"Geração {ger+1}/{geracoes}: Melhor Pontuação = {melhor_pontuacao:.4f}"
            )
    ...
    def visualizar_melhor_mecanismo(self):
        ...
        if self.melhor_individuo is None:
            print(
                "Nenhum melhor indivíduo encontrado ainda. Execute a evolução primeiro."
            )
            return
        ...
        ax.plot(
            caminho_pe[:, 0], caminho_pe[:, 1], "b-", alpha=0.5, label="Caminho do Pé"
        )
        ...
        def atualizar(frame):
            ...
            linhas[0].set_data(
                [pontos["p1"][0], pontos["p2"][0]], [pontos["p1"][1], pontos["p2"][1]]
            )
            linhas[1].set_data(
                [pontos["p2"][0], pontos["p3"][0]], [pontos["p2"][1], pontos["p3"][1]]
            )
            linhas[2].set_data(
                [pontos["p3"][0], pontos["pe"][0]], [pontos["p3"][1], pontos["pe"][1]]
            )
        ...
        ani = animation.FuncAnimation(
            fig, atualizar, frames=np.linspace(0, 360, 72), interval=50, blit=True
        )
...
if __name__ == "__main__":
    ...
    print(
        "Iniciando otimização genética para encontrar proporções ótimas do Strandbeest..."
    )
    ...
    print(
        f"Melhoria: {(pontuacao_jansen - pontuacao_otimizada) / pontuacao_jansen * 100:.2f}%"
    )
    ...
    plt.plot(
        caminho_pe_original[:, 0],
        caminho_pe_original[:, 1],
        "b-",
        linewidth=2,
        label="Design Original de Jansen",
    )

    # Design otimizado
    caminho_pe_otimizado = otimizado.simular_rotacao_completa()
    plt.plot(
        caminho_pe_otimizado[:, 0],
        caminho_pe_otimizado[:, 1],
        "r-",
        linewidth=2,
        label="Design Geneticamente Otimizado",
    )
